...
| `.row`, `.row-dot`, `.row-main`, `.row-from`, `.row-sub`, `.row-snip`, `.row-tail`, `.row-time` (`ui/row.rs`) | `ListRow` (§16) | `ListRow`'s props (`name`, `via`, `subject`, `snippet`, `time`, `tags`, `star`, `strip`) are exactly this row's fields — see `CONSUMING.md` section 6's Lists example. mailo gaps 2: `subject`/`snippet` take `ds::Text` runs for search hits (`marked.rs`'s pieces become `Run`s with `RunTone::Mark`); `on_sender`/`on_time` (`PartHooks`) replace the `data-hc` sender and time spans' handlers; `onpointerenter`/`onpointerleave`/`onpointerdown` the row's (thread card, drag press); `aria_label` the `Open {subject}` label |
| `.strip`, `.fly`, `.floater` (`ui/row.rs:251,296,301`) | `HoverStrip` (§17) | the hover-reveal action row and its `zZ` snooze float. mailo gaps 2: `shown` reveals it on the keyboard's row, `titles: Titles::FromLabel` keeps mailo's `title`s, `expanded` marks the snooze and label buttons' menus; a strip click no longer reaches the row (drop mailo's own `stop_propagation`) |
| `.fmenu` (`ui/menu/mod.rs:58`, `.fmenu.slim` too) | `Menu` with `MenuKind::Rich` or `MenuKind::Slim` | mailo's own `slim` variant is `MenuKind::Slim`; everything else is `MenuKind::Rich`. mailo gaps 2: the composer's `/`, `@` and people menus pass `active: Cursor::Controlled(Some(sel))` and move `sel` from their own field's keys (`on_active` hears the pointer's requests); the label menu's "Create label '…'" reads `onquery`; `Right::Remove(_)` is `MenuRow { trailing: Some(RowAction { icon: Icon::X, .. }), .. }` |
| `.cmdk`, `.cmdk-wrap`, `.cmdk-in` (`ui/command/mod.rs:72-97`) | `CommandPalette<T>` (§25) | mailo gaps 2: `entrance: PaletteEntrance::Opaque` (`cmdk-rise`, no fade) satisfies `the_command_panel_is_opaque_on_its_first_frame`; a row whose title mixes tones or carries mailo's own marks is `MenuEntry::Row(MenuRow { title: Text::Runs(..), .. })`, and a recent search's × is its `trailing: Some(RowAction { icon: Icon::X, .. })` |
| `.hc`, `.hc.beside`, `.hc.tip`, `.hc.side-card` (`ui/hover/cards.rs`) | `HoverCard` + `HoverTarget` (§22) | the three `.hc` modifiers are placement, which `ds::place`/`Placement` computes instead of being hand-picked per call site; the card's contents are `HoverCardPart`s. mailo gaps 2: `.hc.tip` is `HoverKind::Tip` (one line, tooltip-sized, below the time); a target that is itself a list item is `HoverTarget { as_: TargetElement::Li }`; a row's sender and time hook through `ListRow { on_sender, on_time }` |
...
| `.pin.acct`, `.av`, `.n` (`ui/sidebar/panes.rs::AccountTiles`) | `AccountTile` (§27) | `AccountFace` is the data type; `space::avatar_color` stays mailo's (an `AccountId`-keyed colour, not part of `ds::Space`). mailo gaps 2: `mark: MarkStyle` follows the provider-marks setting (`ProvChip`'s `marks`) |
| `.pin.acct.acct-add` (the "+" after the tiles, same file) | `AddAccountTile { title: Some("Add account…"), onclick }` | mailo gaps 2; the dashed ring and its hover are quire's, so `accounts.css` lines 31-32 go |
...
| the Space dots editor (`ui/space_editor/{hue,mod}.rs`) | `SpaceEditor` / `SpaceDot` (§32) | this is the one component whose whole job is a settled design, not a stylesheet: read `space_editor/mod.rs` fully before starting. mailo gaps 2: the `.ed-name` field is `on_rename`, the Motion `Seg` is `motion: Some(MotionChoice { level, on_motion })` over `ds::Motion` (mailo's own three-level `Motion` maps to `Calm`/`Standard`/`Extra`), `measured_in` and `Readout`'s `.check-h` headings are `measured: MeasuredIn::EachScheme`, the presets are named by `Preset::name` (quire has mailo's first eight; the six accent presets stay mailo's if it keeps them). The Theme row stays the editor's own `SegmentedControl` (per-Space theme). What the editor does not draw stays mailo's markup around it: Provider marks, Notifications, the Accounts/Contacts/Rules/Keys rows, the Cancel/Save foot |
...
| `@keyframes pill-up` (`style/composer.css:219`), used by `.sendpill` and mailo's own `.toast` (`style/list.css:125`) | `pill-up` (`Anim::PillUp`) | mailo gaps 3: the same frames under the same name. Delete mailo's `@keyframes` and its `Rule::Keyframes` exception; the `animation:` lines stay as written |
| `@keyframes ring-drain` and `.sendpill .run.countdown { animation: ring-drain 5s … }` (`style/composer.css:226-230`) | `ring-drain` at `var(--t-send-ring) var(--e-linear) forwards` (`Anim::RingDrain`), or `SendPill` itself | mailo gaps 3: `--t-send-ring` is a hold, so Reduced keeps the 5 s; the `RawDuration` and `Keyframes` exceptions go |
| `@keyframes fade-in` and `.scrim` (`style/reader.css:111-112,209`) | `fade-in` (`Anim::FadeIn`) and `opacity: var(--veil)` | mailo gaps 3: `fade-in` ends on `--veil` (.16), the scrim's resting opacity; write the rest opacity as the token too |
| `@keyframes busy` and `.acct-busy` (`style/accounts.css:18-19`) | `busy` (`Anim::Busy`) | mailo gaps 3: the same frames; delete mailo's |
| `@keyframes cmdk-rise` (`style/menus.css:66`), used by the files, contacts, keys, rules and editor sheets | `cmdk-rise` (`Anim::CmdkRise`, mailo gaps 2) | quire's rise is `cmdk-in`'s scale and lift (.93, -12 px, overshoot) without the fade, not mailo's gentler 8 px and .98. Delete mailo's `@keyframes`: under the same name it would shadow quire's for every surface that loads both sheets |
| `.bubble .serif { font-family: Georgia, serif }` (`style/composer.css:157`) | `font-family: var(--font-serif)` | mailo gaps 3: Noto Serif, shipped; the `FontFamily` exception goes |
| `.pin.acct[aria-pressed=false] .av:not(.all)`'s `filter` (`style/`, the `BlitzUnsupported` exception) | `AccountTile` (already mutes an unpressed tile), or `Avatar { muting: AvatarMuting::Muted }` | mailo gaps 3: the muted colour is computed (.55 chroma), no `filter` |
...
| the row's own strip buttons (`ui/row.rs`; the `STRIP_AND_FRAME` `button` exception in `ui/style/exceptions.rs`) | `HoverStrip { on_press, .. }` (§17) | mailo gaps 4: `on_press: EventHandler<ActionId>` fires inside the click before any rect is read, so archive, trash and read act with no layout (the no-layout tests); keep each action's `onclick: EventHandler<Rect>` for snooze and label, whose menus it anchors (it follows `on_press` when the rect resolves). Drop the strip from the exception's reason |
| the hover hooks and `crate::hover::Timer` (`ui/hover/mod.rs`, `cards.rs`; rows, senders, times, pins, Today tabs) | `use_hover_intent()` and `HoverCard { flow }` (§22) | mailo gaps 4: from each hook call `driver.over(HoverKey(..), kind, HoverAnchor::Element(mounted))` (or `HoverAnchor::Rect` from the pointer event), `driver.out()` and `driver.press()`, and draw `HoverCard` for `driver.hub().open().or(hub.leaving())`: the hub is the same machine (450 / 0 warm / 150 / 400 ms). A test with no layout passes `HoverAnchor::Unplaced`, or draws the card with `flow: Flow::Inline`, and asserts it where it stands; nothing reads, fetches or loads |
...
| a raw `button` inside a `<summary>` whose click handler calls `stop_propagation` (a section header's action) | `Button { propagation: Propagation::Stop, .. }` or `IconButton { propagation: Propagation::Stop, .. }` | mailo gaps 5: the press stays at the button and the `<details>` does not toggle, on the webview and on Blitz |
| the `.scrim` under a peeked reader inside a pane (drawn by mailo because quire's floated above the reader) | `Scrim { flow: Flow::Inline, onclose, label }`, rendered in the pane before the reader | mailo gaps 5: the pane must be positioned (`position:relative`); the reader rendered after the scrim sits above it |
| the composer's quoted-message head (a raw `button` with a strong "who" and a faint "when") | `Button { label: Text::Runs(vec![Run::new(who, RunTone::Strong), Run::new(when, RunTone::Faint)]), .. }` | mailo gaps 5: named by the runs' characters unless `aria_label` is given |
| a menu with a visible "Filter…" line (labels, folders, move-to) | `Menu { filter: Filter::Field { placeholder: "Filter…".into() }, onquery, .. }` | mailo gaps 5: the typed query is drawn at the top; keys, `onquery` and the cursor are as under `Filter::Typing` |
| the drop outline on every place while a thread drag is live (`is-drop-ok`-style class before the pointer is over one) | `SidebarItem { drop: DropState::Accepts, .. }`, `DropState::Target` under the pointer | mailo gaps 5: a dashed accent hairline, weaker than the target's fill and scale |
...
### 6.8 Spelling in the composer (2026-09-27): what mailo adds
