...
/// window's shell to paint when a resource lands; give every frame `frame_nav`; and reach the
/// clipboard through the shell.
pub(crate) fn install(
    handle: &NodeHandle,
    setup: &Setup,
    clipboard: &HostClipboard,
    frame_nav: Arc<dyn NavigationProvider>,
) {
    ...
    let frame_net = DsNet::frame(setup.net.clone(), Some(Arc::clone(&waker)));
