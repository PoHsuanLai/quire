//! The three faces, shipped inside the binary as bytes (design/02-TYPE.md section 2).
//!
//! On Blitz the bytes are registered with the renderer's font context once (`ds-native`'s
//! `register_fonts`); nothing about fonts is in the stylesheet. The `webview-fonts` feature
//! keeps mailo's interim path, `@font-face` rules with base64 `data:` URIs generated by
//! `build.rs`, for a consumer still on the webview.
//!
//! The files are Google Fonts' own `latin` and `latin-ext` subsets as WOFF2, copied from mailo
//! with their OFL licences (`assets/fonts/OFL-*.txt`).

use crate::tokens::Family;

/// Upright or italic.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub enum FaceStyle {
    /// Roman.
    Normal,
    /// Italic.
    Italic,
}

/// The weights a face covers: one for a static instance, a range for a variable face.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub struct Weight {
    /// The lightest weight.
    pub min: u16,
    /// The heaviest weight.
    pub max: u16,
}

/// Which unicode subset a file carries.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub enum Subset {
    /// Basic Latin and Latin-1, general punctuation.
    Latin,
    /// Latin Extended.
    LatinExt,
}

/// One font file.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub struct Face {
    /// Which family it belongs to.
    pub family: Family,
    /// Upright or italic.
    pub style: FaceStyle,
    /// The weights it covers.
    pub weight: Weight,
    /// The subset it carries.
    pub subset: Subset,
    /// The file.
    pub bytes: &'static [u8],
}

macro_rules! face {
    ($family:expr, $style:expr, $min:expr, $max:expr, $subset:expr, $file:literal) => {
        Face {
            family: $family,
            style: $style,
            weight: Weight {
                min: $min,
                max: $max,
            },
            subset: $subset,
            bytes: include_bytes!(concat!("../assets/fonts/", $file)),
        }
    };
}

/// Every face the design system ships.
pub static FACES: &[Face] = &[
    face!(
        Family::Display,
        FaceStyle::Normal,
        500,
        800,
        Subset::Latin,
        "bricolage-grotesque-normal-500-800-latin.woff2"
    ),
    face!(
        Family::Display,
        FaceStyle::Normal,
        500,
        800,
        Subset::LatinExt,
        "bricolage-grotesque-normal-500-800-latin-ext.woff2"
    ),
    face!(
        Family::Ui,
        FaceStyle::Normal,
        400,
        700,
        Subset::Latin,
        "karla-normal-400-700-latin.woff2"
    ),
    face!(
        Family::Ui,
        FaceStyle::Normal,
        400,
        700,
        Subset::LatinExt,
        "karla-normal-400-700-latin-ext.woff2"
    ),
    face!(
        Family::Ui,
        FaceStyle::Italic,
        400,
        400,
        Subset::Latin,
        "karla-italic-400-latin.woff2"
    ),
    face!(
        Family::Ui,
        FaceStyle::Italic,
        400,
        400,
        Subset::LatinExt,
        "karla-italic-400-latin-ext.woff2"
    ),
    face!(
        Family::Data,
        FaceStyle::Normal,
        400,
        400,
        Subset::Latin,
        "space-mono-normal-400-latin.woff2"
    ),
    face!(
        Family::Data,
        FaceStyle::Normal,
        400,
        400,
        Subset::LatinExt,
        "space-mono-normal-400-latin-ext.woff2"
    ),
    face!(
        Family::Data,
        FaceStyle::Normal,
        700,
        700,
        Subset::Latin,
        "space-mono-normal-700-latin.woff2"
    ),
    face!(
        Family::Data,
        FaceStyle::Normal,
        700,
        700,
        Subset::LatinExt,
        "space-mono-normal-700-latin-ext.woff2"
    ),
];

/// `@font-face` rules with the faces as base64 `data:` URIs, for a consumer still on the
/// webview. Generated by `build.rs`.
#[cfg(feature = "webview-fonts")]
pub fn font_face_css() -> &'static str {
    include_str!(concat!(env!("OUT_DIR"), "/fonts.css"))
}

#[cfg(test)]
mod tests {
    use super::{FACES, FaceStyle, Subset};
    use crate::tokens::Family;

    #[test]
    fn every_family_ships_both_subsets() {
        // Each (family, style, weight) appears once per subset, and no file is empty.
        for face in FACES {
            assert!(!face.bytes.is_empty(), "{face:?} is empty");
            let twins = FACES
                .iter()
                .filter(|other| {
                    other.family == face.family
                        && other.style == face.style
                        && other.weight == face.weight
                })
                .map(|other| other.subset)
                .collect::<Vec<_>>();
            assert_eq!(
                twins.len(),
                2,
                "{:?} {:?} {:?}",
                face.family,
                face.style,
                face.weight
            );
            assert!(twins.contains(&Subset::Latin) && twins.contains(&Subset::LatinExt));
        }
        for family in [Family::Display, Family::Ui, Family::Data] {
            assert!(
                FACES
                    .iter()
                    .any(|face| face.family == family && face.style == FaceStyle::Normal),
                "{family:?} has no upright face"
            );
        }
    }

    #[test]
    fn every_face_is_woff2() {
        // `wOF2`, the WOFF2 signature. A TTF subset would start 0x00010000; see FINDINGS.
        for face in FACES {
            assert_eq!(face.bytes.get(..4), Some(&b"wOF2"[..]), "{face:?}");
        }
    }
}
