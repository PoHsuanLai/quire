...
/// plate and the layers of its group behind it, an edge panel (notification parts), and a
/// desktop widget's card (sill FINDINGS Q182; a widget tile has no material of its own).
const CARDS: [&str; 6] = [
    ".ds-popover",
    ".ds-sheet",
...
mod tests {
    ...
    fn every_tint_scales_its_own_alpha_by_the_key() {
        ...
        const WANT: &[&str] = &[
            ...
            ".ds[*|data-material][*|data-chrome=transparent] .ds-popover,.ds[*|data-material][*|data-chrome=transparent] .ds-sheet,.ds[*|data-material][*|data-chrome=transparent] .ds-notification-plate,.ds[*|data-material][*|data-chrome=transparent] .ds-notification-layer,.ds[*|data-material][*|data-chrome=transparent] .ds-panel,.ds[*|data-material][*|data-chrome=transparent] .ds-widget[*|data-host=desktop]{background:var(--m-tint-solid);border-color:transparent;box-shadow:var(--m-box);}",
        ];
        for want in WANT {
