...
1. **A "more" glyph.** `Icon::Ellipsis` and `Icon::EllipsisVertical`, Lucide `ellipsis` and
   ...
   without touching these lines. A test locks the circles; the Controls page draws both at the
   row and header sizes and on Strip and Tool buttons. The gallery has no page of every glyph on
   master, so the specimen is its own section.
