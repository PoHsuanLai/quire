...
| Components | clock (display face), `Avatar` (or the user's animated emoji or photo through `UserPicture`, design/25 section 7: Attentive while typing, Wince on a wrong password, Happy on unlock with the accept beat, Asleep with the display off), `TextInput{Boxed}` password, `Button{Primary}` | P |
